...
def make_parser():
    ...
    parser.add_argument(
        "-pt", "--path", default="./datasets/UniRTL/MOT/train/person_41019_5_564", help="path to images or video"
    )
    parser.add_argument(
        "-tm",
        "--track_mode",
...
class UnismotPredictor(object):
    def __init__(
        ...
        # self.rgb_means = (0.375, 0.409, 0.403)
        # self.std = (0.162, 0.161, 0.164)
    ...
    def inference(self, imgi, imgv, imgl, timer, frameID, gt_0):
        ...
        img_info["raw_imgl"] = imgl
        img_info["raw_imgv"] = imgv
        ...
        if frameID == 1:
            ...
            if FTP_RLM is not None:
                ...
                lable_pos = F.interpolate(lable_map, scale_factor=1 / 8, mode="bilinear", align_corners=False).flatten(
                    -2).cuda()
...
def image_demo(predictor, vis_folder, current_time, args):
    if osp.isdir(args.path):
        ...
        RGB_img_list, T_img_list, L_img_list, RGBT_gt, frame0_gt = genConfig(args.path, args.track_node)
        demo_result_name = args.path.split('/')[-1]
    else:
        files = [args.path]
    RGB_img_list.sort()
    T_img_list.sort()
    ...
    for frame_id, img_path in enumerate(zip(T_img_list, RGB_img_list, L_img_list), 1):
        ...
        if outputs[0] is not None:
            ...
            pre_results = np.append(gt_tlbr, 1) if frame_id == 1 else pre_results
            ...
            online_targets = tracker.update(pre_results, [img_info['height'], img_info['width']], exp.test_size, gt_frame0=img_info, img_name=imgi_path, mode='nd')
            id_none += 1
        online_tlwhs = []
        online_ids = []
        online_scores = []
        ...
        if online_targets is not None:
            ...
            online_im, online_imv = plot_tracking(
                img_info['raw_imgi'], online_tlwhs, online_ids, frame_id=frame_id, fps=1. / timer.average_time,
                det_tlbr=bboxes_det, imageinfo=img_info
            )
        ...
        if args.save_image_result:
            ...
            save_folder = osp.join(vis_folder, demo_result_name+timestamp)
    ...
    if args.save_result:
        res_file = osp.join(vis_folder, f"{demo_result_name}.txt")
...
def main(exp, args):
    ...
    predictor = UnismotPredictor(model, exp, trt_file, decoder, args.device, args.fp16, args.useFTP)
