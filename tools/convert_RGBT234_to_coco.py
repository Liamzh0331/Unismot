...
DATA_PATH = 'datasets/RGB-T234'
DATA_ROOT = 'datasets'
OUT_PATH = os.path.join(DATA_ROOT, 'annotations_RGBT234')
SPLITS = ['train_random', 'train_half', 'val_half', 'train']  # --> split training data to train_half and val_half.
HALF_VIDEO = True
CREATE_SPLITTED_ANN = True
CREATE_SPLITTED_DET = True

RGBT234_CLASSES = ('person', 'car', 'bike', 'moto', 'head', 'e-tricycle', 'bus', 'bag', 'truck', 'dog', 'guidepost', 'hotglass',
                   'hotkettle', 'kite', 'soccer', 'stroller', 'toy')
RGBT234_dict = {'person': 1, 'car': 2, 'bike': 3, 'moto': 4, 'head': 5, 'e-tricycle': 6, 'bus': 7, 'bag': 8, 'truck': 9,
                'dog': 10, 'guidepost': 11, 'hotglass': 12, 'hotkettle': 13, 'kite': 14, 'soccer': 15, 'stroller': 16,
                'toy': 17}

classRGBT234 = RGBT234_CLASSES
if __name__ == '__main__':
    ...
    for split in SPLITS:
        ...
        tid_last = -1
        print(seqs)
        for i, clss in enumerate(classRGBT234):
        ...
        for seq in sorted(seqs):
            ...
            if cat_id_name in RGBT234_CLASSES:
                cat_id = RGBT234_dict[cat_id_name]
            else:
            ...
            for i, imgzip in enumerate(zip(T_img_list, RGB_img_list)):
                ...
                image_info = {'file_name': 'RGB-T234/{}/infrared/{}'.format(seq, IR_img),  # image name.
                              'file_name_rgb': 'RGB-T234/{}/visible/{}'.format(seq, rgb_img),
