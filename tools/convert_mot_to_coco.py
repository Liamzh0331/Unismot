...
import json
import cv2
from unismot.data import UniRTL_CLASSES

# Use the same script for MOT16
DATA_PATH = 'datasets/UniRTL'
OUT_PATH = os.path.join(DATA_PATH, 'MOT', 'annotations')
SPLITS = ['train_half', 'val_half', 'train', 'test']  # --> split training data to train_half and val_half.
HALF_VIDEO = True
CREATE_SPLITTED_ANN = True
CREATE_SPLITTED_DET = True
classUniRTL = UniRTL_CLASSES
...
if __name__ == '__main__':
    ...
    for split in SPLITS:
        if split == "test":
            data_path = os.path.join(DATA_PATH, 'MOT', 'test')
        else:
            data_path = os.path.join(DATA_PATH, 'MOT', 'train')
        ...
        tid_curr = 0
        tid_last = -1
        for i, clss in enumerate(classUniRTL):
        ...
        for seq in sorted(seqs):
            ...
            if 'MOT' in DATA_PATH and split == 'test':
                continue
            # seq = 'person_41014_1_452'
            video_cnt += 1  # video sequence number.
            out['videos'].append({'id': video_cnt, 'file_name': seq})
            seq_path = os.path.join(data_path, seq)
            img_path = os.path.join(seq_path, 'IR')
            ann_path = os.path.join(seq_path, 'gt/gt_mot.txt')
            ...
            for i in range(num_images):
                if i < image_range[0] or i > image_range[1]:
                    continue
                img = cv2.imread(os.path.join(data_path, '{}/IR/i{:04d}.jpg'.format(seq, i + 1)))
                height, width = img.shape[:2]
                image_info = {'file_name': 'MOT/train/{}/IR/i{:04d}.jpg'.format(seq, i + 1),  # image name.
                              'file_name_rgb': 'MOT/train/{}/rgb/v{:04d}.jpg'.format(seq, i + 1),  # image name.
                              'file_name_low': 'MOT/train/{}/low/l{:04d}.jpg'.format(seq, i + 1),  # image name.
            ...
            if split != 'test':
                ...
                for i in range(anns.shape[0]):
                    ...
                    cat_id = int(anns[i][7])
                    ann_cnt += 1
                    if not ('SOT' in DATA_PATH):
                        #if not (float(anns[i][8]) >= 0.25):  # visibility.
                            #continue
                        if not (int(anns[i][6]) == 1):  # whether ignore.
                            continue
                        if int(anns[i][7]) in [36, 0]:  # Non-person
                            continue
                        else:
                            category_id = int(anns[i][7])  # pedestrian(non-static)
