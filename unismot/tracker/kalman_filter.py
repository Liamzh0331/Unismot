...
def is_positive_definite(matrix):
    ...
    if np.any(np.isinf(matrix)) or np.any(np.isnan(matrix)):
        # print("The matrix contains inf or NaN values.")
        return False
    try:
...
class KalmanFilter(object):
    ...
    def __init__(self):
        ...
        self._std_weight_position = 1. / 10   # 20
        self._std_weight_velocity = 1. / 100  # 160
    ...
    def update(self, mean, covariance, measurement):
        ...
        projected_cov = ensure_positive_definite(projected_cov)
        try:
            ...
            U, s, Vt = np.linalg.svd(projected_cov)
            pseudo_inverse = np.dot(Vt.T, np.dot(np.diag(1 / s), U.T))
            kalman_gain = np.dot(covariance, np.dot(self._update_mat.T, pseudo_inverse))
    ...
    def euclidean_distance(self, mean, covariance, measurements, only_position=False, metric='euclidean'):
        mean, covariance = self.project(mean, covariance)
        if only_position:
            mean, covariance = mean[:2], covariance[:2, :2]
            measurements = measurements[:, :2]
        d = measurements - mean
        if metric == 'euclidean':
            squared_euclidean = np.sum(d * d, axis=1)  # calculate the squared Euclidean distance
