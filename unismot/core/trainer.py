...
from unismot.data import DataPrefetcher
from unismot.utils import (
    MeterBuffer,
    ModelEMA,
    all_reduce_norm,
    get_model_info,
...
class Trainer:
    ...
    def train_one_iter(self):
        ...
        inps, inpl, inpv, targets = self.prefetcher.next()
        track_ids = targets[:, :, 5]
        targets = targets[:, :, :5]
        inps = inps.to(self.data_type)
        inpv = inpv.to(self.data_type)
        inpl = inpl.to(self.data_type)
        ...
        with torch.cuda.amp.autocast(enabled=self.amp_training):
            if self.nlow:
                outputs = self.model(inps, inpv, inpl, targets)
            else:
                outputs = self.model(inps, inpv, xl=None, targets=targets)
    ...
    def before_train(self):
        ...
        logger.info(
            "Model Summary: {}".format(get_model_info(model, self.exp.test_size, self.args.nlow))
        )
    ...
    def evaluate_and_save_model(self):
        ...
        ap50_95, ap50, summary = self.exp.eval(
            evalmodel, self.evaluator, self.is_distributed, self.nlow
        )
        self.model.train()
        if self.rank == 0:
