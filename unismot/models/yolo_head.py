...
from unismot.utils import bboxes_iou
...
class YOLOXHead(nn.Module):
    ...
    def forward(self, xin, labels=None, imgs=None, FTP=None):
        outputs = []
        origin_preds = []
        x_shifts = []
        y_shifts = []
        ...
        if FTP is None:
            ...
            for k, (cls_conv, reg_conv, stride_this_level, x, ftp) in enumerate(
                    zip(self.cls_convs, self.reg_convs, self.strides, xin, FTP)
            ):
                x = self.stems[k](x)
                if self.fuse_method == 'sum':
                    x = x + (ftp * self.betasum).half()
                cls_x = x
                reg_x = x
